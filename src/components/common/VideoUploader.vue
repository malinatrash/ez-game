...
const emit = defineEmits<{ 'update:modelValue': [id: string | undefined, filename?: string] }>()
...
async function onPick(file: File) {
  ...
  emit('update:modelValue', id, file.name)
}
...
<template>
  <div class="uploader">
    ...
    <div v-if="url" class="preview-wrap">
      <video :src="url" controls class="preview" />
      <UiButton class="remove" variant="danger" size="sm" icon-only title="Убрать" @click="clear">✕</UiButton>
    </div>
    <UiFileButton v-else accept="video/*" @pick="onPick">🎬 Загрузить видео</UiFileButton>
  </div>
</template>
